DeriveSoilParams<-function(catch, 
                     DEM, 
  ...
  #st_crs(DEM_latlon) == st_crs(catch_v)
  
  if (projection(DEM)==projection(catch_v) | projection(DEM)=="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0") 
    #only do reprojection if necessary 
    DEM_latlon =DEM else
    {  
      print("reprojecting DEM, may take some time ...")
      DEM_latlon <- try(projectRaster(DEM, crs = wgs), silent = TRUE)   
  ...
  #check if DEM_latlon and catch_v cover the same area, i.e. that they have at least some overlap
  b_dem   = st_as_sfc(st_bbox(DEM_latlon))
  b_catch = st_as_sfc(st_bbox(catch_v))
  b_catch = st_transform(b_catch, crs = st_crs(b_dem)) #strangely, this step is needed for the successful use of st_intersect()
  
  if (!st_intersects(b_dem, 
                     b_catch, sparse = FALSE))
  {
    stop("DEM and catchment do not overlap in space, check projections and extents.")
  }
  #Crop DEM to the size of the catchment's bbox
  DEM<-crop(DEM_latlon, st_bbox(catch_v))
  rm(DEM_latlon)
  
  
  #Divide study area into tiles
  v<-ceiling(nrow(DEM)/max_cells) #number of vertical tiles
  h<-ceiling(ncol(DEM)/max_cells) #number of horizontal tiles
...
#required grids  
required_grids = c(
  depth =paste0(dtb_dir, "/depth_$no$.tif"), 
  soils =soil_classes_infile,
  clay  =paste0(soilgrids_dir, "/clay_sd$no$.tif"), 
  silt  =paste0(soilgrids_dir, "/silt_sd$no$.tif"),
  sand  =paste0(soilgrids_dir, "/sand_sd$no$.tif"),
  coarse=paste0(soilgrids_dir, "/cfvo_sd$no$.tif"),
  bulkD =paste0(soilgrids_dir, "/bdod_sd$no$.tif"),
  om    =paste0(soilgrids_dir, "/soc_sd$no$.tif"),
  ph    =paste0(soilgrids_dir, "/phh2o_sd$no$.tif"),
  cec   =paste0(soilgrids_dir, "/cec_sd$no$.tif")
)
 ...
 for (a in start$a:v){
   for (b in 1:h){
     ...
     if (a == start$a & b <= start$b) next  #fast-forward to specified beginning
      ...
      dem<-mask(x=dem, mask=catch_v) #set cells outside the catchment to NA
...
#      depth<-aggregate(depth, fact=f_a)
      ...
      for (soillayer in 1:6)
      {
        ...
        ptf_props=data.frame(theta_pwp = euptf::predict.ptf(newdata=euptf_attributes, ptf="PTF12"))
        
        #Saturated water content
        ptf_props$theta_s<- euptf::predict.ptf(newdata=euptf_attributes, ptf="PTF06")
        
        #Saturated hydraulic conductivity (horizons) [cm/d]    	
        ptf_props$ks<- (10^(euptf::predict.ptf(newdata=euptf_attributes, ptf="PTF17")))*10
        
        #Calculate theta_s/2.5/1.8,suction, h_b and lambda with ptf.rawls####  
        
        tt = soilwaterptf::ptf.rawls(soilprop=soil_attributes, h=316, parameters=c("theta", "S_f", "theta_r", "h_b", "lambda"))
        ...
        ptf_props$fk63 = soilwaterptf::ptf.rawls(soilprop=soil_attributes, h=63, parameters="theta")[,"theta"]
      ...
      write.table(x=data.frame("a"=a, "b"=b), file="last_tile.txt", row.names = F, sep=" \t", quote=FALSE)
   }
   }
  ...
  soil_means<- data.table::data.table(soil_sum_collected)
  #aggregate the rows of the dataframe soil_means by column soil_means$soil_id using weighted mean (weight from cellcount)
  library(dplyr)
  soil_means_aggregated <- soil_means %>%
    group_by(soil_id) %>%
    summarise(across(
      # Select all but the grouping and weight columns for weighted mean calculation
      .cols = !c(cellcount), 
      # Apply weighted mean calculation
      .fns = ~ sum(. * cellcount) / sum(cellcount), 
      # Naming convention for new weighted mean columns
      .names = "{.col}"
    ))
  #soil_means<- computeWeightedMeans(data_table=soil_means, weight=soil_means$cellcount, by=soil_means$soil_id )

  soil_means2<-data.frame(soil_means_aggregated)
  rm(soil_means)
  write.table(x=soil_means2, file="soil_sum_weighted.txt", sep=" \t", quote=FALSE)
  ...
  #For table "r_soil_contains_particles" (only topsoil is considered, because it is used from surface erosion only)  
  ...
  #Create output map (for use in lumpR) ####
  #Read in all soil-id tiles, with new soil ids for alluvium and then merge them to one map
  

  l_soils<-list()
  x<-list.files(dirname(soil_classes_outfile), pattern="^soils_[0-9].*\\.tif$") #avoid reading in an old merged map or other files
  for (i in 1:length(x))
    l_soils[[i]]<-raster(paste0(dirname(soil_classes_outfile), "/", x[i]))
  ...
  dir.create(dirname(soil_classes_outfile), showWarnings = FALSE)
  writeRaster(m_soils, file=soil_classes_outfile, overwrite=T, datatype="INT2S")
